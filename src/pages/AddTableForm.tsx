...
import { createTable, queryClient } from '../util/http';
...
export default function AddTableForm({onClose}: {onClose: () => void}) {
  ...
  return (
    <Box sx={{display: 'flex', flexDirection: 'column', height: '100%', maxHeight: '90vh', overflow: 'hidden' }}>
      ...
      <form id="addTableForm" onSubmit={handleTableForm} style={{
        flex: 1,
        // minHeight: 0,        
        overflowY: 'auto',  
        display: 'flex', 
        ...
        //   flexGrow: 1, 
          ...
          <Box>
            <InputLabel htmlFor='tableNumber' sx={{color: 'black', marginBottom: '5px'}}>Table Number <span style={{color: 'red'}}>*</span></InputLabel>
            <OutlinedInput error={!!errors.tableNumber} id='tableNumber' name='tableNumber' placeholder='Table Number' />
            {errors.tableNumber && <FormHelperText error>{errors.tableNumber}</FormHelperText>}
          </Box>  
          <Box>
